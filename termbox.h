...
struct tb_event {
	...
	uint8_t mod; 
	uint16_t key;//特殊按键的值
	uint32_t ch; //普通按键的unicode值，可以用utf8_unicode_to_char来转换成char
	int32_t w;   //当前终端窗口的宽度
	int32_t h;   //当前终端窗口的高度
};
...
SO_IMPORT int tb_init();      //初始化窗口
SO_IMPORT void tb_shutdown(); //关闭窗口

SO_IMPORT unsigned int tb_width();  //返回窗口宽
SO_IMPORT unsigned int tb_height(); //返回窗口高度

SO_IMPORT void tb_clear();   //清除缓冲区
SO_IMPORT void tb_present(); //和终端同步内部缓冲区

#define TB_HIDE_CURSOR -1
SO_IMPORT void tb_set_cursor(int cx, int cy);  //设置光标的位置
...
SO_IMPORT void tb_blit(unsigned int x, unsigned int y, unsigned int w, unsigned int h, const struct tb_cell *cells); //drawing functions

#define TB_INPUT_ESC 1
#define TB_INPUT_ALT 2
/* with 0 returns current input mode */
SO_IMPORT int tb_select_input_mode(int mode); //改变输入模式

#define TB_EVENT_KEY 1
#define TB_EVENT_RESIZE 2 //调整窗口大小

SO_IMPORT int tb_peek_event(struct tb_event *event, unsigned int timeout); //peek a keyboard event
SO_IMPORT int tb_poll_event(struct tb_event *event);  //wait for a keyboard event
...
SO_IMPORT int utf8_char_to_unicode(uint32_t *out, const char *c);//成功返回1
SO_IMPORT int utf8_unicode_to_char(char *out, uint32_t c);       //成功返回1
